  ...
  if (utils.endsWith(file?.originalname.toLowerCase(), '.xml')) {
    ...
    if (((file?.buffer) != null) && utils.isChallengeEnabled(challenges.deprecatedInterfaceChallenge)) { // XXE attacks in Docker/Heroku containers regularly cause "segfault" crashes
      ...
      try {
        ...
        const xmlDoc = vm.runInContext('libxml.parseXml(data, { noblanks: true, noent: false, dtdload: false, nocdata: true })', sandbox, { timeout: 2000 })
  ...
  if (utils.endsWith(file?.originalname.toLowerCase(), '.yml') || utils.endsWith(file?.originalname.toLowerCase(), '.yaml')) {
    ...
    if (((file?.buffer) != null) && utils.isChallengeEnabled(challenges.deprecatedInterfaceChallenge)) {
      ...
      try {
        ...
        vm.createContext(sandbox)
        const yamlString = vm.runInContext('JSON.stringify(yaml.parse(data))', sandbox, { timeout: 2000 })
        res.status(410)
